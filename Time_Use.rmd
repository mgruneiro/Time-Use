...
## Building model for traveling

```{r, results="hide"}
set.seed(84735)
travel_model1 <- stan_glm(ACT_TRAVEL ~ AGE + HH_NUMOWNKIDS +UHRSWORKT + EARNWEEK, 
                             data = data_2019,
                             family = gaussian,
                             chains = 4,
                             iter = 1000*2)
...
```{r}
mcmc_trace(travel_model1)
mcmc_dens_overlay(travel_model1)
```
...
```{r}
set.seed(84735)
pp_check(travel_model1, nreps = 50)
```
